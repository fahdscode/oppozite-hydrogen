...
interface ShopifyProductCardProps {
  product: any; // Using any for flexibility during migration, eventually use proper type
  index?: number;
}
...
export const ShopifyProductCard = ({ product, index = 0 }: ShopifyProductCardProps) => {
  ...
  // Handle both {node} wrapper and direct node formats
  const node = product.node || product;

  // Cycle images on hover
  useEffect(() => {
    let interval: NodeJS.Timeout;
    if (isHovered && !selectedVariantImage && node?.images?.edges && node.images.edges.length > 1) {
